...
result_gd = optimize(tf.f, tf.gradient!, tf.meta[:start], GradientDescent(), Optim.Options(f_tol=1e-6))
result_lbfgs = optimize(tf.f, tf.gradient!, tf.meta[:start], LBFGS(), Optim.Options(f_tol=1e-6))
